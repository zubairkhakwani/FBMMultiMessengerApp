...
namespace FBMMultiMessenger.Database.Services
{
    public class MessageDbService
    {
        ...
        public async Task<List<GeChatMessagesHttpResponse>> GetChatMessages(int chatId, int currentUserId)
        {
            ...
            {
                var replyResult = GetMessageReply(new MessageReplyRequest
                {
                ...
                return new GeChatMessagesHttpResponse()
                {
                    ChatMessageId = x.Id,
                    ChatId = chatId,
        ...
        public MessageReplyResult? GetMessageReply(MessageReplyRequest request)
        {
            ...
            if (chatMessage.IsVideoMessage || chatMessage.IsImageMessage)
            {
                ...
                result.Attachments = mediaUrls?.Select(x => new MessageReplyFileHttpResponse()
                {
                    Url = x
                }).ToList();
            }
